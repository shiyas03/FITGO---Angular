...
import { EventData } from '../../services/trainer.interface';
...
export class DetailsComponent implements OnDestroy {
  ...
  onFileSelected(event: Event) {
    ...
    this.selectedCertificates = <FileList>(event.target as HTMLInputElement).files;
  ...
  sendDetails() {
    ...
    for (let i = 0; i < this.selectedCertificates.length; i++) {
      formData.append('details', this.selectedCertificates[i], this.selectedCertificates[i].name);
    }
    this.subscription1 = this.trainerServices.detailsUpload(formData, id).subscribe((res) => {
      if (res.success == false) {
        const dialogRef: MatDialogRef<ErrorDialogComponent> = this.dialog.open(ErrorDialogComponent)
        this.subscription2 = dialogRef.afterClosed().subscribe((res) => {
          this.router.navigate(['/trainer/register'])
        })
      } else {
        const dialogRef: MatDialogRef<SuccessComponent> = this.dialog.open(SuccessComponent)
        this.subscription2 = dialogRef.afterClosed().subscribe((res) => {
